    ...
    <div className="h-full flex flex-col md:flex-row gap-8 overflow-y-auto pb-8">
      ...
      <div className="w-full md:w-1/2 flex flex-col gap-6">
        ...
        <p className="text-lg text-gray-700 font-medium">
          Families spend a significant portion of their grocery bill on cleaning and personal care products.
        </p>

        {/* Updated Grid: Full color images, no overlays, labels underneath */}
          ...
          {Object.entries(productImages).map(([label, fileName]) => (
            <div key={label} className="flex flex-col items-center gap-2">
              <div className="relative w-full h-20 md:h-24 rounded-xl overflow-hidden border border-gray-100 shadow-sm">
                <Image 
                  src={`/images/presentation/${fileName}`}
                  alt={label}
                  fill
              ...
              <span className="text-[10px] font-bold text-blue-900 uppercase tracking-tight text-center leading-none">
                {label}
              </span>
            </div>
          ))}
